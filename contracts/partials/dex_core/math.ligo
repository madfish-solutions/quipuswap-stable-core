...
function get_A(
  ...
  block {
    ...
    if Tezos.now < t1
    then {
      const t_num = nat_or_error(Tezos.now - t0, Errors.timestamp_error);
    ...
    while abs(tmp.d - tmp.prev_d) > 1n
      block {
        ...
        tmp.d := (a_nn * sum_c / Constants.a_precision + d_P * tokens_count) * tmp.d / (
          nat_or_error(a_nn - Constants.a_precision, Errors.wrong_precision) * tmp.d / Constants.a_precision + (tokens_count + 1n) * d_P
...
function calc_y(
  ...
  block {
    ...
    while abs(tmp.y - tmp.prev_y) > 1n
      block {
        tmp.prev_y := tmp.y;
        tmp.y := (tmp.y * tmp.y + c) / nat_or_error(2 * tmp.y + b - d, Errors.nat_error);
      }
  } with tmp.y
...
function get_y_D(
  ...
  block {
    ...
    assert_with_error(i >= 0n, Errors.wrong_index); // dev: i below zero
    assert_with_error(i < tokens_count, Errors.wrong_index);  // dev: i above N_COINS
...
function calc_withdraw_one_coin(
  ...
  block {
    ...
    var dy := nat_or_error(xp_red_i - get_y_D(amp, i, xp_reduced, d1, pool), Errors.nat_error);
    const t_i = unwrap(pool.tokens_info[i], Errors.wrong_index);
    ...
    const dy_0 = nat_or_error(xp_i - new_y, Errors.nat_error) / precisions_i;  //# w/o s.fee
    total_supply := nat_or_error(pool.total_supply - token_amount, Errors.low_total_supply);
  ...
  block {
    ...
    function balance_it(
      ...
      block {
        ...
        const old_info = unwrap(init_tokens_info[i], Errors.wrong_index);
...
function perform_swap(
  ...
  block {
    ...
    const t_i     = unwrap(pool.tokens_info[i], Errors.wrong_index);
    const t_j     = unwrap(pool.tokens_info[j], Errors.wrong_index);
  ...
  block {
    ...
    then {
      const balanced = balance_inputs(
        ...
        new_tokens_info,
        d1,
        unwrap(s.tokens[params.pool_id], Errors.pool_not_listed),
        pool.fee,
    ...
    const user_key = (Tezos.sender, params.pool_id);
