...
[@inline] function nip_off_fees(
  ...
  const token_info      : tkn_inf_t)
                        : tkn_inf_t is
  token_info with record [ reserves = nat_or_error(token_info.reserves - sum_wo_lp_fee(fees), Errors.low_reserves); ]
...
function get_token_by_id(
  ...
  block {
    const tokens = unwrap(map_entry, Errors.pool_not_listed);
    ...
    const (
      ...
      total_staked
        ...
        | Remove -> (
            ...
            Tezos.self_address,
            Tezos.sender,
            nat_or_error(p_s_acc.total_staked - shares, Errors.wrong_shares_out)
            )
        end;
    function upd_former(
